"""1D/2D discrete ell^2 regression (collocation) with tanh RFM / ELM.

Mirrors the structure of L2minimization1d2dRFM.py:
...
def npz_path_1d(m):
    return DATA_DIR / f'elm_1d_sin_m_{m}_collocation.npz'


def npz_path_2d(m):
    return DATA_DIR / f'elm_2d_sin_m_{m}_collocation.npz'


def run_exp_1d(m, R_m_list=None, trial_num=5, base_seed=BASE_SEED):
    """1D collocation / discrete ell^2 regression; saves npz."""
    ...
    for R_m in R_m_list:
        ...
        for n in neuron_num_arr.astype(int):
            ...
            for trial in range(trial_num):
                ...
                my_model.fc2.weight.data[0, :] = torch.tensor(sol).view(-1)
                with torch.no_grad():
            ...
            print(f'1d-reg m={m} R={R_m} n={n}: L2={mean_err_l2_list[-1]:.3e}', flush=True)
...
def run_exp_2d(m, R_m_list=None, trial_num=5, base_seed=BASE_SEED):
    """2D collocation / discrete ell^2 regression; saves npz."""
    R_m_list = list(R_m_list or R_M_LIST)
    Nx, order = 50, 3
    ...
    for R_m in R_m_list:
        ...
        for n in neuron_num_arr.astype(int):
            ...
            mean_err_l2_list.append(float(torch.mean(err_l2_trial)))
            seed_n_list.append(seeds_trial)
            print(f'2d-reg m={m} R={R_m} n={n}: L2={mean_err_l2_list[-1]:.3e}', flush=True)
        mean_err_rows.append(mean_err_l2_list)
        seed_rows.append(seed_n_list)
...
def plot_1d(m_list=None, out_name='ELM_1d_l2reg.png'):
...
def plot_2d(m_list=None, out_name='ELM_2d_l2reg.png'):
    m_list = list(m_list or M_LIST)
    _apply_plot_style()
    fig, axes = plt.subplots(1, 3, figsize=(12, 3.6), dpi=400)
    for ax, m in zip(axes, m_list):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='1D/2D tanh discrete ell^2 regression (collocation) RFM',
    )
    parser.add_argument(
