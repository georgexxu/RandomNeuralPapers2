"""Plot collocation / discrete ell^2-regression error vs neuron count.

Produces one figure per ReLU power (relu1 and relu2).
Reads collocation-l2regression-{d}d-relu{k}.npz in this folder.
...
for relu_k in (1, 2):
    ...
    for ax, d in zip(axes.flat, range(1, 7)):
        path = folder / f'collocation-l2regression-{d}d-relu{relu_k}.npz'
        data = np.load(path)
        ...
        if relu_k == 1:
            title = rf'$\ell^2$-regression. ReLU. $\Pi_{{i=1}}^{d}\sin(m\pi x)$, $m={m_target}$'
        else:
            title = (
                rf'$\ell^2$-regression. ReLU$^{relu_k}$. '
    ...
    fig.tight_layout()
    out = folder / f'l2regression_convergence_vs_neurons_relu{relu_k}.png'
