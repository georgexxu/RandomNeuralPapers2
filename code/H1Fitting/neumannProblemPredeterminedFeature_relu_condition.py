...
# Comment out a block below to skip that dimension.

# 1D 
# u(x) = sin(πx/2), (ω,b) uniform on S^1 
def target(x):
    z = ((pi/2)**2 + 1)*torch.sin(pi/2*x[:,0:1])
    return z
...
neuron_num_arr = np.array([2**i for i in range(3,10)])  # 8, ..., 512
actual_neuron_list = []
...
for neuron_num in neuron_num_arr:
    ...
    A, _ = assemble_matrix_H1_explicit_assemble_efficient_general_dim(my_model,target,None,integration_weights,integration_points,torch.tensor([]),torch.tensor([]),activation='relu',solver='direct')
    condition_number = galerkin_condition_number(A)
    print("condition number: ", condition_number)
    condition_number_list.append(condition_number)
    actual_neuron_list.append(my_model.fc1.bias.size(0))
    np.savez(f'{folder}Neumann-problem-Predetermined-Feature-{d}d-relu{relu_k}.npz', actual_neuron_arr=actual_neuron_list, condition_number_arr=condition_number_list)


# 2D 
# u(x) = sin(πx/2) sin(πy/2), Fibonacci grid on S^2 
def target(x):
...
for neuron_num in neuron_num_arr:
    ...
    my_model = remove_redundant_neuron(my_model.cpu(), dims=d, choice=2).to(device)
    A, _ = assemble_matrix_H1_explicit_assemble_efficient_general_dim(my_model,target,None,integration_weights,integration_points,torch.tensor([]),torch.tensor([]),activation='relu',solver='direct',memory=2**27)
    condition_number = galerkin_condition_number(A)
    print("condition number: ", condition_number)
    condition_number_list.append(condition_number)
    actual_neuron_list.append(my_model.fc1.bias.size(0))
...
# 4D 
def target(x):
    z = (4 * (pi/2)**2 + 1)*torch.prod(torch.sin(pi/2 * x), dim=1, keepdim=True)
    return z
...
neuron_num_arr = np.array([2**i for i in range(3,9)])  # 8, ..., 256
actual_neuron_list = []
condition_number_list = []
torch.manual_seed(0)
for neuron_num in neuron_num_arr:
...
# 6D 
def target(x):
    z = (6 * (pi/2)**2 + 1)*torch.prod(torch.sin(pi/2 * x), dim=1, keepdim=True)
    return z
